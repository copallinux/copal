...
**Subject:** integration of diinki's *Linux Antiquity* Hyprland theme as Copal's "full monty" install level (stage 17)
**Date:** 2026-08-25
...
(stage 4, the "medium" level) and Wayland (stage 17, the "full monty") is
...
| foot | packaged (community) | **the terminal `$terminal` opens.** Wayland-native, renders on the CPU. Upstream ships no config for it; stage 17 writes a `foot.ini` translating the theme's `kitty/hades.conf` palette (§V-E) |
...
For reference, the complete chain stage 17 performs, in order. This is the
...
`x11`, stage 17 writes `wayland`; the last stage run owns the console —
...
| `copal-launcher` | written by stage 17 | Super+Space / D / Z: `copal-menu` → wofi drun → dmenu. The quickshell radial launcher is no longer preferred — it would be a third menu that knows about neither pane |
| `copal-shot` | written by stage 17 | Super+Shift+S: hyprshot → grim+slurp |
| `copal-wallpaper` | written by stage 17 | hyprpaper → swaybg, upstream's image |
| generated `hyprland.conf` | `ANTIQHYPR` heredoc in stage 17 | the Lua translation (§V-C) |
...
2. **A persisted settings object**, written to
   ...
   | `execCommands.{terminal,files}` | `kitty` / `nemo` | what the shell launches; on a machine where stage 17 fell back to `pcmanfm`, change `files` here too |
...
Stage 17 therefore closes four layers the theme does not carry, after the
copy and before it claims the session:

| Layer | What stage 17 does | Why it is not in the vendored tree |
|---|---|---|
...
machine rather than the same one without a compositor. Stage 17 recolours it
...
plainly because it is the checklist stage 17 implements:
...
- **mako**: `makoctl reload`.
- **Everything at once**: re-running stage 17 reinstalls the vendored configs
...
wrapper). Stage 17 follows the same grain, which should keep future
...
stage 17 to acquisition + packages + session.
