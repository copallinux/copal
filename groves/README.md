# `groves/` — what a day looks like, written down

One directory per grove. `groves/example/` is the template; `copal grove init`
copies it to `groves/<name>/` with the grove's name, its login account and its
...
```
groves/<name>/
  grove.toml          the room: which nodes, how their power switches,
  ...
  inventory/
    copal_grove.py    the host list, out of discovery. Never edited by hand
...
```
copal grove scene            what scenes exist
copal grove scene wake       power on, wait for beacons, mount, ready
copal grove scene show       screens on, today's program up
copal grove scene reset      between visitors: restore home, start clean
copal grove scene rest       screens dark, machines up
copal grove scene sleep      flush logs, halt, and confirm each one went
```
...
inventory cannot wait for itself to fill. **Recording** what the grove is now
...
not as `copal-grove`, which has a forced command by design, and a forced
command cannot run a module. Host key checking stays **on**: `copal grove
trust` put one `@cert-authority` line in `known_hosts`, so every node in the
grove validates with no prompt, and a host key that *changes* is an error
...
```sh
ansible-galaxy collection install -r groves/<name>/requirements.yml
```
...
without it is still fully drivable by `copal grove run`, which needs nothing
